...
class Player(Entity):
    def __init__(self, position, SPEED = 2, velocity = (0, 0, 0), MAXJUMP = 1, gravity = 1, controls = "wasd", **kwargs):
        super().__init__(
            model = "cube",
            position = position,
            scale = (1, 1, 1), 
        ...
        Text(text = '+', origin = (0, 0), size = 0.05, position = (mouse))
        self.time.disable()
    ...
    def jump(self):
        self.velocity_y = self.jump_height * 50
        self.jump_count += 1
